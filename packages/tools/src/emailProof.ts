...
export class EmailProof {
    ...
    private _randomString(): string {
        return (this._randomSeed++) + "" + new Date().getTime() + "" + Math.floor(Math.random() * 100000);
    }
    ...
    > {
        ...
        await this._execPromise(`node ${join(this.generateWitnessJsDir, "generate_witness.js")} ${wasmFile} ${circuitInputFile} ${witnessFile}`);
        ...
        try {
            if (this._rapidsnarkProverBin) {
                ...
                await this._execPromise(`${this._rapidsnarkProverBin} ${zkeyFile} ${witnessFile} ${proofFile} ${publicFile}`);
                ...
                let _publicSignals: string[] = JSON.parse(readFileSync(publicFile).toString('utf-8'));
