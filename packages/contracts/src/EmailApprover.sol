...
contract EmailApprover is Initializable, UUPSUpgradeable, IERC1271 {
    ...
    function getEmailApproverInfo() external view returns (DKIMRegistry, Groth16Verifier, bytes32) {
        return (dkimRegistry, verifier, senderCommitment);
    }

    function _isValidProof(uint256[8] memory proof, bytes32 pubkeyHash, bytes32 senderDomainHash, bytes32 approvedHash)
        internal
        view
        returns (bool)
    {
        ...
        return verifier.verifyProof(
            [proof[0], proof[1]], [[proof[2], proof[3]], [proof[4], proof[5]]], [proof[6], proof[7]], signals
        );
    }

    function approve(uint256[8] memory proof, bytes32 pubkeyHash, bytes32 senderDomainHash, bytes32 approvedHash)
        public
    {
    ...
    function isValidSignature(bytes32 hash, bytes memory signature) external view returns (bytes4 magicValue) {
        ...
        // decode signature
        (uint256[8] memory proof, bytes32 pubkeyHash, bytes32 senderDomainHash) =
            abi.decode(signature, (uint256[8], bytes32, bytes32));
        return _isValidProof(proof, pubkeyHash, senderDomainHash, hash) ? MAGICVALUE : INVALID_ID;
    }
